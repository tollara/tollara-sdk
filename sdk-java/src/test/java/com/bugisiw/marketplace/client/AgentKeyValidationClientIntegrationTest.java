...
/**
 * Integration tests for AgentKeyValidationClient against WireMock stubs of the Agent Hub Core API
...
class AgentKeyValidationClientIntegrationTest {
    ...
    void validateAgentKey_returnsResult_whenCoreReturns200WithValidHmac() throws Exception {
        // Per API spec §2.1: response has X-Marketplace-Signature = HMAC(responseBody + timestamp, agentSecret)
        String responseBody = """
        ...
        wireMock.stubFor(
                post(urlPathEqualTo("/api/v1/agent-keys/validate"))
                        ...
                        .willReturn(aResponse()
                                ...
                                .withHeader("X-Marketplace-Signature", signature)
                                .withHeader("X-Marketplace-Timestamp", timestamp)
        ...
        wireMock.stubFor(
                post(urlPathEqualTo("/api/v1/agent-keys/validate"))
                        .willReturn(aResponse()
                                .withStatus(200)
                                .withHeader("Content-Type", "application/json")
                                .withHeader("X-Marketplace-Signature", badSignature)
                                .withHeader("X-Marketplace-Timestamp", timestamp)
                                .withBody(responseBody))
        );
