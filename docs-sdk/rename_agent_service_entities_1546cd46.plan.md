...
overview: Hard rename Service->Service, Developer->Developer, AgentUser->User across backend (Java + DB + Cognito + Redis + Stripe metadata), frontend, SDK, and docs. Brand "Tollara" is preserved. No backward-compat aliases (pre-prod).
todos:
  - id: phase1_spec
    ...
    status: completed
  - id: phase2_db
    content: "Phase 2: Rename Postgres database tollara_db -> tollara_db (init scripts, all application*.yml, docker compose, scripts, e2e/load-test configs, Copilot DB addons) AND add Flyway migrations to rename tables/columns/FKs/indexes in core, usage, messaging databases; update postgres init SQL."
    status: completed
  - id: phase3_packages
    content: "Phase 3: Rename Java groupId 'com.tollara' AND package roots 'com.tollara' / 'com.tollara' -> 'com.tollara' in every build.gradle and source folder. Fix usage-service flat package by introducing 'com.tollara.usage.*'. Update @ComponentScan in TestConfig and logging.level.* in all application yml files. Build green per module."
    status: completed
  - id: phase4_common
  ...
  - id: phase6_samples
    ...
    status: completed
  - id: phase7_sdk
    content: "Phase 7: Update reference Java SDK code (validateServiceKey->validateServiceKey, ServiceKey->ServiceKey types) and SDK README files (JS/.NET/Java/Python) with new package names and method signatures for export to tollara-sdk repo."
    status: completed
  ...
  - id: phase8_frontend_copy
    ...
    status: completed
  - id: phase9_docs
    content: "Phase 9: Sweep docs/*.md (~30 files) for entity terminology; update businessContext.txt narrative; rename and update postman/agent_hub.postman_collection.json -> tollara.postman_collection.json with all new paths."
    status: completed
  - id: phase10_verify
...
- Keep the `Tollara` brand intact: `TollaraClient`, `X-Tollara-*`, `TOLLARA_*`, `tollara.ai`, logos.
- Update [docs-sdk/MAIN-SDK-API-SPEC.md](docs-sdk/MAIN-SDK-API-SPEC.md) (including the change log) so the standalone `tollara-sdk` repo can be brought into line in one pass.
...
- SDK packages: `@tollara/service-sdk`->`@tollara/service-sdk`; `Tollara.ServiceSdk`->`Tollara.ServiceSdk`; `com.tollara:service-sdk`->`com.tollara:service-sdk`; `tollara-service-sdk` (PyPI)->`tollara-service-sdk`
...
- Postgres database name: `tollara_db` -> `tollara_db` (legacy name from pre-rebrand "Service Hub"; aligns with the `Tollara` brand). Postgres user `tollaramaster` already brand-aligned, keep.
- Java group IDs and package roots: both `com.bugisiw, com.tollara` and `com.tollara` -> `com.tollara`. This sits alongside the existing SDK namespace `com.tollara.client.*` (no collision because the service subpackages are `core`, `gateway`, `usage`, `security`, `messaging`, `common`, `proxiedservice`, `nonproxiedservice`, `proxiedllm`, `proxiedmcp`, `e2e`, `loadtest`).
- Usage-service package consistency fix (rolled into the package rename pass): currently flat (`com.tollara.controller`, `.service`, `.model`, `.config`) -> `com.tollara.usage.*` so it matches the per-service convention used by every other service.
...
What stays unchanged: `TollaraClient`, `TollaraHeaders`, `TollaraSdkConfig`, `TollaraRequestVerifier`, `X-Tollara-*` HTTP headers, `TOLLARA_*` env vars, `tollara.ai`, `TollaraLogo_*.png`. After Phase 3 the service code also lives under `com.tollara.*` - the SDK's `com.tollara.client.*` namespace is unchanged.
...
flowchart TB
  ...
  db --> pkgs[3. Java groupId + package rename to com.tollara]
...
**2a. Database name rename (`tollara_db` -> `tollara_db`)**
...
- Update Postgres init script [docker/postgres/init/01-create-database.sql](docker/postgres/init/01-create-database.sql) - change `CREATE DATABASE tollara_db` to `tollara_db`.
...
- Update Copilot DB addons: rename `copilot/environments/agent-hub-ppe/addons/agenthub-db.yml` and `copilot/environments/agent-hub-prod/addons/agenthub-db.yml` to `tollara-db.yml` and update DB name inside. (`copilot/environments/tollara-prod/addons/tollara-db.yml` already uses the new name.) Folder names of the environments themselves stay - only the addon file + content changes.
...
### Phase 3 - Java groupId + package rename to `com.tollara`
...
- Update `group = 'com.tollara'` to `group = 'com.tollara'` in every `build.gradle` listed by the survey: root [build.gradle](build.gradle) (if present), [lib/common-utils/build.gradle](lib/common-utils/build.gradle), all `services/*/build.gradle`, all `services/*/build.gradle`, [e2e-tests-java/build.gradle](e2e-tests-java/build.gradle), [load-tests/build.gradle](load-tests/build.gradle).

**3b. Source directory + package declarations**

- Rename source folders mechanically:
  - `services/<service>/src/{main,test,integration-test}/java/com/bugisiw/marketplace/<sub>` -> `.../java/com/tollara/<sub>`
  - `services/core-service/src/{main,test,integration-test}/java/com/marketplace/core` -> `.../java/com/tollara/core`
  - `lib/common-utils/src/main/java/com/bugisiw/marketplace/common` -> `.../com/tollara/common`
  - `services/*/src/main/java/com/bugisiw/marketplace/<proxiedagent|proxiedllm|proxiedmcp|nonproxiedagent>` -> `.../com/tollara/<same>` (the entity rename in Phase 6 then renames `proxiedagent` -> `proxiedservice` and `nonproxiedagent` -> `nonproxiedservice`).
  - `e2e-tests-java/src/test/java/com/bugisiw/marketplace/e2e` -> `.../com/tollara/e2e`.
  - `load-tests/src/{main,gatling}/java/com/bugisiw/marketplace/loadtest` -> `.../com/tollara/loadtest`.
- **Usage-service consistency fix** in this same pass: move `services/usage-service/src/{main,test,integration-test}/java/com/bugisiw/marketplace/{controller,service,model,config,...}` to `.../com/tollara/usage/{controller,service,model,config,...}`. This brings it in line with `com.tollara.gateway.*`, `com.tollara.security.*`, `com.tollara.messaging.*`. Spring Boot's main class will move to `com/tollara/usage/UsageServiceApplication.java`, ensuring `@SpringBootApplication` package scanning still finds everything.
...
- Update [services/core-service/src/integration-test/java/com/marketplace/core/config/TestConfig.java](services/core-service/src/integration-test/java/com/marketplace/core/config/TestConfig.java): `@ComponentScan(basePackages = "com.tollara.core")` -> `"com.tollara.core"`.
- Sweep all `services/*/src/main/resources/application*.yml` and `src/{integration-test,test}/resources/application-test.yml` (~28 files) for `logging.level.com.bugisiw.*`, `logging.level.com.tollara.*`, and any Spring property values that embed package strings; replace with `com.tollara.*`.
...
- Rename package `com.tollara.common.model.service` -> `com.tollara.common.model.service`.
- Rename classes: `Service`, `Developer`, `ServiceKey`, `ServiceProduct*`, `ServiceProfile*`, `ServiceNotFoundException`, `ServiceRegistrationRequest`, `RegisterRequest` (`userType` constants), [lib/common-utils/src/main/java/com/tollara/common/cache/SubscriptionCacheKeys.java](lib/common-utils/src/main/java/com/tollara/common/cache/SubscriptionCacheKeys.java), [lib/common-utils/src/main/java/com/tollara/common/util/GatewayHmacUserContext.java](lib/common-utils/src/main/java/com/tollara/common/util/GatewayHmacUserContext.java) - keep `Tollara` brand strings here, rename only entity references (`serviceSecret`->`serviceSecret`).
- `lib/common-utils/src/main/java/com/tollara/common/exception/ServiceNotFoundException.java` -> `ServiceNotFoundException.java`.
- Rebuild: `gradle :lib:common-utils:build`.

### Phase 5 - Microservices entity rename (per service)

For each service, rename Java packages, classes, JPA `@Entity`/`@Table` mappings (must match Phase 2 schema), `@RequestMapping` paths, DTO fields, and Spring `@PreAuthorize`/`SecurityConfig` rules. All file paths below use the new `com.tollara.*` roots from Phase 3.

- security-service:
  - [services/security-service/src/main/java/com/tollara/security/service/AuthService.java](services/security-service/src/main/java/com/tollara/security/service/AuthService.java): `userType` allowed values `DEVELOPER`/`USER` -> `DEVELOPER`/`USER`; Cognito group strings.
...
- core-service:
  - Package rename: `com.tollara.core.service.*` -> `com.tollara.core.service.*` (note: module name `core-service` stays).
  ...
  - Billing: [services/core-service/src/main/java/com/tollara/core/billing/controller/CouponController.java](services/core-service/src/main/java/com/tollara/core/billing/controller/CouponController.java) base path `/services`->`/services`; [services/core-service/src/main/java/com/tollara/core/billing/controller/BillingController.java](services/core-service/src/main/java/com/tollara/core/billing/controller/BillingController.java) `/subscriptions/service/{serviceId}`->`/subscriptions/service/{serviceId}`, `/developer/refunds`->`/developer/refunds`; `TransactionController` `/service/{serviceId}`->`/service/{serviceId}`.
  - Stripe metadata in [services/core-service/src/main/java/com/tollara/core/billing/service/StripeService.java](services/core-service/src/main/java/com/tollara/core/billing/service/StripeService.java) and [services/core-service/src/main/java/com/tollara/core/billing/service/TransactionService.java](services/core-service/src/main/java/com/tollara/core/billing/service/TransactionService.java): keys `serviceId`/`developer_id`/`serviceProductId` -> `serviceId`/`developer_id`/`serviceProductId`.
...
- usage-service (already at `com.tollara.usage.*` after Phase 3):
...
- Internal Java packages: `com.tollara.proxiedagent` -> `com.tollara.proxiedservice`, `com.tollara.nonproxiedagent` -> `com.tollara.nonproxiedservice`.
- Update READMEs in each subfolder.

### Phase 7 - Reference SDK code + SDK READMEs

- Java reference SDK under `client/java/` (and any companion under `e2e-test-sdk/`): rename classes (`ServiceKeyValidationClient`->`ServiceKeyValidationClient`, `TollaraRequestVerifier` keeps brand) and methods (`validateServiceKey`->`validateServiceKey`, `reportUsage` keeps name; argument `serviceId`/`serviceSecret` -> `serviceId`/`serviceSecret`).
...
- Update [docs/sdk-callers-and-backends.md](docs/sdk-callers-and-backends.md), [docs/sdk-java-readme.md](docs/sdk-java-readme.md), [docs/sdk-repo-implementation-prompt.md](docs/sdk-repo-implementation-prompt.md), [docs/sdk-repo-project-context.md](docs/sdk-repo-project-context.md) so they describe new package names and method signatures for the tollara-sdk repo to mirror.
- Note in plan: actual publishing of `@tollara/service-sdk` etc. happens in the separate `tollara-sdk` repo (out of scope here); this phase only fixes the contract docs and the reference Java client used by tests in this repo.
...
- [postman/agent_hub.postman_collection.json](postman/agent_hub.postman_collection.json): rename to `postman/tollara.postman_collection.json`; update all `/services`, `/developers`, `/service/:serviceId/endpoint/...`, `/billing/subscriptions/service/:serviceId`, body fields. Add `/service-keys` paths if currently missing.
...
- Run this test from the e2e Java suite (`e2e-tests-java`) with a fresh DB so Flyway runs cleanly, using the following parameters:
  - **.\gradlew.bat test -DTEST_DEVELOPER_EMAIL=**"[test-developer-1776889791564@example.com](mailto:test-developer-1776889791564@example.com)" **-DTEST_DEVELOPER_PASSWORD="TestPassword123!" -DPLAYWRIGHT_HEADED=true -DINVOCATION_DELAY_MS=5 --tests "com.tollara.e2e.SubscriptionMonthlyProxiedOverageTest"**
...
- Confirm SDK contract by running `e2e-test-sdk/run-sdk-contract-e2e.ps1` once the tollara-sdk repo is also updated (out-of-repo step; flag for follow-up).
...
- Keep `Tollara` brand tokens unchanged everywhere - reviewers should explicitly check headers, env vars, class prefixes, logos.
- DB rename `tollara_db` -> `tollara_db`: dev/test DBs will be dropped and recreated from init scripts and Flyway migrations (pre-prod, no data to preserve). For any deployed PPE/PROD environment, this becomes a separate cutover (RDS rename, Copilot redeploy) - not in this PR.
...
- Repo-wide ripgrep for the entity tokens (excluding the `Tollara` brand) returns zero meaningful matches for any of:
  - `\bService\b`
  - `\bagentId\b`
  - `\bDeveloper\b`
  - `\bagent_owner\b`
