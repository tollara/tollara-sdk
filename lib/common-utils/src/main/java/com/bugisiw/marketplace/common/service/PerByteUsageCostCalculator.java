...
/**
 * Implementation of UsageCostCalculator for byte-based billing.
 * Same formula as PerRequestUsageCostCalculator but uses perByteBaseUnits, perByteMaxUnits, perByteTimeUnit.
 */
@Slf4j
public class PerByteUsageCostCalculator implements UsageCostCalculator {
    ...
    public UsageCalculation calculateCost(Subscription subscription,
        ...
        if (subscription.getMeasurementType() != MeasurementType.PER_BYTE ||
            ...
            throw new IllegalArgumentException("PerByteUsageCostCalculator can only handle byte-based SUBSCRIPTION or USAGE_POSTPAID subscriptions");
        }

        BigDecimal baseUnits = subscription.getPerByteBaseUnits() != null ? subscription.getPerByteBaseUnits() : BigDecimal.ZERO;
        BigDecimal maxUnits = subscription.getPerByteMaxUnits() != null ? subscription.getPerByteMaxUnits() : new BigDecimal(Long.MAX_VALUE);
