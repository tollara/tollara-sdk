...
overview: Port Java’s unified `TollaraClient` (env-based config, path prefixes, optional split bases), configurable usage report path, and inbound “verify + user context” convenience to each non-Java SDK where HTTP clients already exist; add verifier-only parity plus README/env alignment where HTTP is out of scope. Add or extend automated tests per SDK and tighten CI where it is currently smoke-only.
todos:
  ...
  - id: unified-client-tier1
    content: Implement TollaraClient (or equivalent) with TOLLARA_* env + URL defaults + split bases in Python, JS, Rust, .NET + integration-style tests
    status: completed
  - id: tier2-docs-env
...
- `[sdk-java/src/main/java/com/tollara/client/TollaraClient.java](sdk-java/src/main/java/com/tollara/client/TollaraClient.java)` — `TOLLARA_API_URL`, `TOLLARA_AGENT_ID`, `TOLLARA_AGENT_SECRET`, default prefixes (`/api/v1`, `/api`, `/api/usage`), optional `coreApiUrl` / `gatewayApiUrl` / `usageApiUrl`, builder explicit values **override** env.
- `[sdk-java/src/main/java/com/tollara/client/UsageServiceClient.java](sdk-java/src/main/java/com/tollara/client/UsageServiceClient.java)` — report URL `{usageBase}{usagePathPrefix}/report` with default prefix `/api/usage`.
- `[sdk-java/src/main/java/com/tollara/client/TollaraRequestVerifier.java](sdk-java/src/main/java/com/tollara/client/TollaraRequestVerifier.java)` — `verifyInboundHmacAndGetUserContext` → empty/absent when invalid, trusted context only when valid.
...
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [sdk-python](sdk-python) | `[validation_client.py](sdk-python/src/tollara_agent_sdk/validation_client.py)`, `[usage_client.py](sdk-python/src/tollara_agent_sdk/usage_client.py)` (`/api/usage/report` hard-coded ~L185), `[gateway_client.py](sdk-python/src/tollara_agent_sdk/gateway_client.py)` | New `client.py` (or `tollara_client.py`) with `TollaraClient` / `from_env()` reading `os.environ`; compose URLs like Java’s `[TollaraUrls](sdk-java/src/main/java/com/tollara/client/TollaraUrls.java)`; extend `report_usage_at` (and callers) with `usage_path_prefix` defaulting to `/api/usage`; re-export from `[__init__.py](sdk-python/src/tollara_agent_sdk/__init__.py)`. |
| [sdk-js](sdk-js)         | `[validationClient.ts](sdk-js/src/validationClient.ts)`, `[usageClient.ts](sdk-js/src/usageClient.ts)`, `[gatewayClient.ts](sdk-js/src/gatewayClient.ts)`                                                                                                                      | New `tollaraClient.ts` exporting `createTollaraClient(options)` (or class) with `process.env.TOLLARA_*` when fields omitted; same URL rules; `reportUsage` params gain optional `usagePathPrefix`; wire `[index.ts](sdk-js/src/index.ts)`.                                                                                                                                                |
| [sdk-rust](sdk-rust)     | `[validation_client.rs](sdk-rust/src/validation_client.rs)`, `[usage_client.rs](sdk-rust/src/usage_client.rs)`, `[gateway_client.rs](sdk-rust/src/gateway_client.rs)` behind `feature = "http"`                                                                                | New `tollara_client.rs` (same feature gate): struct + builder, `std::env::var` for the three names, defaults for path prefixes; add `usage_path_prefix` to `report_usage` path construction in `[usage_client.rs](sdk-rust/src/usage_client.rs)`; export from `[lib.rs](sdk-rust/src/lib.rs)`.                                                                                               |
| [sdk-dotnet](sdk-dotnet) | `[ValidationClient.cs](sdk-dotnet/ValidationClient.cs)`, `[UsageClient.cs](sdk-dotnet/UsageClient.cs)`, `[GatewayClient.cs](sdk-dotnet/GatewayClient.cs)`                                                                                                                      | New `TollaraClient.cs` + options record: resolve from `Environment.GetEnvironmentVariable` when properties null/empty; same URL join rules; extend usage report URL builder with prefix; optional thin wrappers calling existing static/async methods.                                                                                                                                        |
...
| [sdk-php](sdk-php)   | `[Verifier.php](sdk-php/src/Verifier.php)`                          | Add static method e.g. `verifyInboundHmacAndGetUserContext` returning `?array` or small value object: `null` if invalid, else same shape as `userContextFromHeaders`. Document `TOLLARA_`* in `[sdk-php/README.md](sdk-php/README.md)`.                                                                   |
| [sdk-ruby](sdk-ruby) | `[lib/tollara_agent_sdk.rb](sdk-ruby/lib/tollara_agent_sdk.rb)` | Add `verify_signature_from_headers_and_user_context` (or shorter name) returning `nil` on failure, else hash like `user_context_from_headers`. Document env vars in `[sdk-ruby/README.md](sdk-ruby/README.md)`.                                                                                             |
...
- **Python**: `Optional[UserContext]` from `[verifier.py](sdk-python/src/tollara_agent_sdk/verifier.py)`; export in `__init__.py`.
...
| Area                        | What to add                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Verify + context**        | Happy path: valid HMAC → non-empty context with expected fields. Failure: bad signature → empty/null. Mirror `[TollaraRequestVerifierTest](sdk-java/src/test/java/com/tollara/client/TollaraRequestVerifierTest.java)` patterns.                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **Unified client (Tier 1)** | **Build validation**: missing API URL and env → error; missing secret → error; optional agent id omitted → still builds. **Integration-style** (match `[TollaraClientIntegrationTest](sdk-java/src/test/java/com/tollara/client/TollaraClientIntegrationTest.java)`): one origin, stub `GET .../api/requests/.../status`, `POST .../api/usage/report`; optional second case with custom usage prefix `/usage/api/v1/report`. Use ecosystem tools: Python `pytest` + `responses`/`httpx` mock or local HTTP server; JS existing test runner + `fetch` mock; Rust `wiremock` dev-dep or `tokio::test` with mock server; .NET `HttpMessageHandler` fake or WireMock.Net if already justified. |
