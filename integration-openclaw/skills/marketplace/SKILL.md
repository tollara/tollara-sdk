# Marketplace (Agent Hub) – Caller

When the user or the agent needs to **call an agent hosted on Agent Hub**, use the **marketplace_call_agent** tool.

## When to use

- User asks to invoke an external agent, run a marketplace agent, or call an API that is exposed as an Agent Hub agent.
- You need to run a specific agent by `agentId` and `endpointId` (or the user provides a key that maps to an agent).

## Tool: marketplace_call_agent

- **agentId** (required): The agent ID on Agent Hub.
...
2. Call `marketplace_call_agent` with `agentId`, `endpointId`, and `body: { "text": "..." }`.
