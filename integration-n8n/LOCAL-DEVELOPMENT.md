...
1. Create a **public** GitHub repo `tollara/n8n-nodes-tollara` (its non-`.github` contents are force-synced each release).
2. Add the mirror's publish workflow to that repo **by hand**, at `.github/workflows/publish.yml`, with the exact contents of `integration-n8n/mirror-template/publish.yml`. Easiest via the GitHub web UI (**Add file → Create new file**), which creates the `main` branch. The sync deliberately never touches `.github/` (see below), so this file is only ever managed here.
3. Add a repo secret **`MIRROR_PUSH_TOKEN`** in `tollara/tollara-sdk` — a **fine-grained PAT** scoped to `tollara/n8n-nodes-tollara` with **Contents: Read and write** only. It does **not** need workflow permission (the sync excludes `.github/`); fine-grained tokens can't push workflow files anyway.
4. Configure npm **Trusted Publisher** for `n8n-nodes-tollara` → repo **`tollara/n8n-nodes-tollara`**, workflow file `publish.yml`. (This replaces the previous `tollara/tollara-sdk` trusted-publisher entry.)
5. Ensure both repos are **public** — npm provenance from GitHub Actions is rejected for private repos (`422 … Unsupported GitHub Actions source repository visibility: "private"`).

The sync workflow rsyncs the payload into the mirror with `--exclude='.github'`, so the manually-committed `publish.yml` is preserved across releases and the push token needs no workflow scope. The mirror's `package.json` (repository, homepage, scripts) is generated by `scripts/build-mirror.mjs`; edit the source under `integration-n8n/` and the mirror workflow at `mirror-template/publish.yml`, never the mirror repo directly.
